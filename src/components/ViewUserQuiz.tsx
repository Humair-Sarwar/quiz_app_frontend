...
const ViewUserQuiz: React.FC<ViewUserQuizProps> = ({ handleClosePopup, id }) => {
      ...
      >
        <div className="bg-white rounded-[32px] overflow-hidden shadow-[0_32px_128px_-16px_rgba(0,0,0,0.2)] w-full max-w-2xl max-h-[90vh] flex flex-col relative border border-slate-100">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-8">
            {/* Quick Stats Ribbon (Optional - looks very modern) */}
            <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Status</p>
                    <p className="text-sm font-bold text-slate-700 flex items-center gap-2">
                        <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span> Active Result
                    </p>
                </div>
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
