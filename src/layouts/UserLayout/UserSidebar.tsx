  ...
  const navGroups = [
    {
      ...
      links: [
        { path: "/user/quiz-list", label: "Solved Quizzes", icon: FaListCheck },
      ],
    },
    {
      title: "Preferences",
    ...
    <>
      ...
      <div className="lg:hidden flex items-center justify-between bg-white p-4 rounded-3xl shadow-sm border border-slate-100 mb-6">
        <div className="flex items-center gap-3">
          <img
            src={data?.data?.image ? data?.data?.image : no_image}
            alt="User"
            className="w-10 h-10 rounded-xl object-cover"
          />
          <span className="font-black text-slate-800 text-sm mr-4">
            Dashboard
          </span>
        </div>
        <button
  ...
  return (
    <div className="bg-white rounded-[2.5rem] z-0! relative p-6 h-full lg:p-0 overflow-y-auto max-h-screen lg:max-h-none shadow-2xl lg:shadow-none border border-slate-100 lg:border-none">
      ...
      {navGroups.map((group, groupIdx) => (
        <div
          key={group.title}
          className={groupIdx !== 0 ? "mt-10 lg:mt-12" : ""}
        >
          ...
          <ul className="space-y-1.5">
            {group.links.map((link: any) => (
              <li key={link.path}>
                ...
                >
                  <link.icon
                    className={`text-lg transition-all duration-500 relative z-10 group-hover:scale-110`}
                  />
      ...
      <div className="mt-12 p-5 bg-[#ff5b07] rounded-[2rem] relative overflow-hidden group shadow-lg shadow-orange-200">
        ...
        <p className="text-white/80 text-[10px] font-black uppercase tracking-widest relative z-10">
          Current Rank
        </p>
        <h3 className="text-white text-xl font-black mt-1 relative z-10">
          Gold League
        </h3>
      ...
      <div className="mt-8 pt-6 border-t border-slate-100 mb-4">
        <button
          ...
          className="w-full cursor-pointer flex items-center gap-3 px-5 py-4 rounded-[1.25rem] text-slate-400 font-bold text-[13px] hover:bg-red-50 hover:text-red-500 transition-all group"
        >
